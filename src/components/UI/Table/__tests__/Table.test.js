...
const component = (props) => (
  <Table sortable sortKey="title" sortOrder={-1} columns={DATA.columns} rows={DATA.data} prefixClassName="test" {...props}/>
)

test('Smoke', async () => {
  const { queryAllByTestId, container } = render(component());
  expect(queryAllByTestId('table').length).toBe(1);
  expect(container.getElementsByClassName('test-body-row').length).toBe(DATA.data.length);
});
...
test('Hidden Row', async () => {
  ...
  expect(container.getElementsByClassName('test-body-row').length).toBe(DATA.dataWithHidden.length - 1);
})
