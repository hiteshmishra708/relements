...
test('Next page', async () => {
  ...
  const { getByTestId, container, rerender } = render(component({ prefixClassName: 'test', onChange: mockFn }));
...
test('Previous page', async () => {
  const mockFn = jest.fn();
  const { getByTestId, container, rerender } = render(component({ prefixClassName: 'test', onChange: mockFn, initialSlide: 3 }));
...
test('Center Mode', async () => {
  const mockFn = jest.fn();
  const { queryAllByTestId, container } = render(component({ prefixClassName: 'test', onChange: mockFn, initialSlide: 2, centerMode: true }));
  expect(queryAllByTestId('slider').length).toBe(1)
  const nextPageButton = container.getElementsByClassName('test-arrow-right')[0];
  fireEvent.click(nextPageButton);
  expect(mockFn).toHaveBeenCalledTimes(1);
