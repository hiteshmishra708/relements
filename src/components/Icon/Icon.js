...
export default class Icon extends React.Component {
  ...
  render() {
    ...
    if (!src.default && !iconType) return null;
    const IconSvg = src.default ? src.default : getIcon(iconType);
    return (
      <div
        ref={(DOMElement) => {
        ...
        className={`${styles.icon} ${className} ${otherClassName}`}
      ...
      >
        ...
        {tooltip ? (
          <ComponentTooltip
            ...
            tooltipArrowSmall
          >
            <div className={styles.iconTooltip}>{tooltip}</div>
          </ComponentTooltip>
        ) : null}
      </div>
    );
